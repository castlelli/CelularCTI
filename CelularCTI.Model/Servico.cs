...
namespace CelularCTI.Model
{
    public static class Servico
    {
        ...
        public static Fabricante ObjFabricante(ref NpgsqlDataReader dtr)
        {
            ...
            fab.Id_Fabricante = (int)Convert.ToInt64(dtr["id_fabricante"]);
            fab.Nome = dtr["nome"].ToString();
            return fab;
        }

        //Os métodos de objetos servem para criar objetos locais para atender requisições
        ...
        public static Cliente ObjCliente(ref NpgsqlDataReader dr)
        {
            Cliente c = new Cliente();
            c.Id_cliente = Convert.ToInt64(dr["id_cliente"]);
        ...
        public static void Salvar(Aparelho ap)
        {
            ...
            if (ap.Id_Aparelho == 0)
            {
                sql = "INSERT INTO aparelho " +
                    "(id_fabricante, modelo, largura, altura, espessura, peso, quantidade, preco, desconto) " +
                    "VALUES ( " + 
            ...
            else
            {
                sql = "UPDATE aparelho SET " +
                    ...
                    "modelo = '" + ap.Modelo + "'," +
                    "largura = " + ap.Largura.ToString().Replace(',', '.') + "," + //Replace('Isso', 'Por isso')
        ...
        public static Aparelho BuscarAparelho(Int64 id)
        {
            ...
            dtr.Read();
            aparelho = ObjAparelho(ref dtr); //"Monta" o objeto aparelho para ser exibido no DataReader
            dtr.Close();
        ...
        public static List<Aparelho> BuscarAparelho(string modelo)
        {
            ...
            //Colocar um IF -> Caso não encontre o ID!
            sql = "SELECT aparelho.*, fabricante.* " +
                "FROM aparelho INNER JOIN fabricante " +
                "ON aparelho.id_fabricante = fabricante.id_fabricante " +
                "WHERE aparelho.modelo ILIKE '%" + modelo + "%' " + //ILIKE -> Desconsidera maiúscula ou minúscula
        ...
        public static List<Aparelho> BuscarAparelho()
        {
            ...
            sql = "SELECT aparelho.*, fabricante.* " +
                "FROM aparelho INNER JOIN fabricante " +
                "ON aparelho.id_fabricante = fabricante.id_fabricante " +
                "WHERE aparelho.modelo ILIKE '%%' " + //ILIKE -> Desconsidera maiúscula ou minúscula
                "ORDER BY aparelho.modelo";
        ...
        public static List<Aparelho> BuscarAparelhosFab(Fabricante f)
        {
            List<Aparelho> aparelhos = new List<Aparelho>();
            NpgsqlDataReader dtr = ConexaoBanco.Selecionar(
        ...
        public static List<Fabricante> BuscarFabricante(Int64 id)
        {
            List<Fabricante> fabricantes = new List<Fabricante>();
            NpgsqlDataReader dtr = ConexaoBanco.Selecionar("SELECT * FROM fabricante WHERE id = "+id.ToString());
            while (dtr.Read())
